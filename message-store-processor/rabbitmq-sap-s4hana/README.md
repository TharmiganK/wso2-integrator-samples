...
**2. Start the RabbitMQ broker and Datadog Agent.** From this directory:

```bash
docker compose up -d
```

This starts RabbitMQ with the `sales-orders`, `sales-orders-dlq` and `sales-orders-res` queues already declared (loaded from `rabbitmq/definitions.json`). The management UI is at [http://localhost:15672](http://localhost:15672) (`guest`/`guest`). The Datadog Agent starts alongside the broker; verify it picked up the OpenMetrics check and OTLP receiver:
