# Sales Order Processor (Solace)

The **consumer** of the [Solace → SAP S/4HANA](../README.md) sample. A
...
| `connections.bal` | The three stores (`solace:MessageStore` from the bundled `wso2/solace` package) and the SAP client (with a small `retryConfig` for transient errors). |
...
Needs the Solace broker and Temporal (both come up with `docker compose up -d`)
